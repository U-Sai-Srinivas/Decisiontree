...
# Key-managed so Streamlit owns the value directly — avoids dropping the last
# keystroke when another widget triggers a rerun. _load_spec seeds session_state.title
# before this widget renders, so template/upload loads still update the box.
...
with edit_tab:
    ...
    if not is_tree:
        ...
        node_cfg.pop("value")
    # The keyed editor owns its own edit state across reruns. We must NOT write its
    # result back into st.session_state.nodes_df (the value we feed as `data`): doing
    # so re-applies the widget's stored edits on top of a changed base and reverts the
    # cell you just typed. The base only changes when a template/file is loaded.
...
# Use the editors' live return values (nodes_df / edges_df) so the preview reflects the
# most recent edit immediately, without a write-back loop through session_state.
